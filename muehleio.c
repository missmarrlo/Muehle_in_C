...
void welcome(int pieces, int jumps_allowed)
{
        ...
        printf("Ein Spieler spielt mit den Steinen, die in blau mit einem %c dargestellt werden, und der andere spielt mit denen, die in pink mit einem %c dargestellt werden. Du kannst zwischen zwei Spielmodi waehlen. Du kannst zusammen mit einem anderen Spieler vor dem Computer spielen oder dich fuer ein Spiel gegen unseren hyperintelligenten Computer entscheiden. Hierbei spielt der Computer immer mit den %c-Steinen.\n\n", SPIELSTEIN_O, SPIELSTEIN_X, SPIELSTEIN_X);
}
...
void print_stone(char stone)
{
        if (stone == SPIELSTEIN_X) {
                printf(COLOUR_X "%c" COLOUR_RESET, stone);
        } else {
                if (stone == SPIELSTEIN_O) {
                        printf(COLOUR_O "%c" COLOUR_RESET, stone);
                }
                else {
                        printf("%c", stone);
                }
...
{
        printf(ANSI_COLOR_LIME "          _     _ _____ _      _ _      _ _ _  \n" COLOUR_RESET);
        printf(ANSI_COLOR_LIME "         \\ \\   / /  ____\\ \\   / /\\ \\   / / | | \n" COLOUR_RESET);
        printf(ANSI_COLOR_LIME "          \\ \\_/ /| |__   \\ \\_/ /  \\ \\_/ /| | | \n" COLOUR_RESET);
        printf(ANSI_COLOR_LIME "           \\   / |  __|   \\   /    \\   / | | | \n" COLOUR_RESET);
        printf(ANSI_COLOR_LIME "            | |  | |____   | |      | |  |_|_| \n" COLOUR_RESET);
        printf(ANSI_COLOR_LIME "            |_|  |______|  |_|      |_|  (_|_) \n" COLOUR_RESET);
        printf("\n\n");
